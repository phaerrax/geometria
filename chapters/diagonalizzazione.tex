...
Se $v$ è un autovettore associato a $\lambda$, anche i suoi multipli per uno scalare sono a loro volta autovettori: infatti se $T(v)=\lambda v$ e $k\in K$ segue per la linearità di $T$ che $T(kv)=kT(v)=k\lambda v$, cioè qualsiasi multiplo $kv$ è un autovettore di $T$ associato a $\lambda$.
...
\end{definizione}
%Siano $\lambda_1,\dots,\lambda_k$ autovalori di $T$, e $e_1,\dots,e_{\alpha_1}$ gli autovettori associati a $\lambda_1$, $e_{\alpha_1+1},\dots,e_{\alpha_2}$ associati a $\lambda_2$, quindi in generale $e_{\alpha_{j-1}+1},\dots,e_{\alpha_{j-1}+\alpha_j}$ sono gli autovettori associati a $\lambda_j$, con $j=1,\dots,k$. Si ha che $T( e_1)=\lambda_1 e_1$, e se $\alpha_1>1$ allora $T( e_2)=\lambda_1 e_2$, fino a $T( e_{\alpha_1})=\lambda_1 e_{\alpha_1}$, $T( e_{\alpha_1+1})=\lambda_2 e_{\alpha_1+1}$ e così via. Gli autovalori $\lambda_j$ sono quindi gli unici elementi non nulli, nelle righe e nelle colonne dove sono posti, della matrice associata a $T$. Tale matrice è diagonale, ossia gli unici elementi non nulli si trovano sulla diagonale e sono proprio gli autovalori. Dato che, come si era dimostrato in precedenza, gli autovalori non variano al cambiare della base, tale matrice è unica, a meno ovviamente di permutazioni.
...
In tale base, $T$ è rappresentato da una matrice $D$ diagonale, poich\'e $T(e_i)=\lambda_ie_i$: in ogni autospazio $V_i$ l'endomorfismo agisce infatti come $\lambda_i I_{\gamma_i}$, ossia come un multiplo dell'identità, quindi
	...
	\end{equation*}
	con $g\in K[x]$.
	Ma $K[x]$ è un dominio a fattorizzazione unica % vedi teorema...
	...
	Riunendo le base di tutti gli autospazi, otteniamo ancora un insieme $\mathcal I$ linearmente indipendente, in base al teorema \ref{unioni-linearmente-indipendente} % da sistemare
...
\end{proof}
% Il seguente corollario non è già compreso nel teorema precedente?
%\begin{corollario}
%	Sia $T\in \End(V)$, con $\dim V=m<+\infty$.
%	Se $T$ ha $m$ autovalori distinti, allora $T$ è diagonalizzabile.
%\end{corollario}
%\begin{proof}
%	La dimostrazione è immediata, dato che esistono $m=\dim V$ linearmente indipendenti (per il teorema \ref{t:autovettori-linearmente-indipendenti}, dato che sono associati ad autovalori distinti), dunque essi formano una base di quindi $T$ è diagonalizzabile.
%\end{proof}
\begin{teorema}
...
\end{proof}
% Sappiamo che esiste sempre il polinomio minimo per un generico endomorfismo ed è il generatore monico dell'ideale $I_T$; considerando l'insieme $S_T$ precedentemente definito è semplice verificare che si può dare un'analoga definizione, poichè valutando il polinomio monico in $T$ esso mi darà sicuramente lo spazio $W$ (quindi rispetta la definzione di $T$-invariante \ref{d:t-invarianza}), inoltre è il minore dei polinomi ad essere un ideale (rispetta cioè l'ultimo teorema dimostrato).
...
\begin{lemma}\label{l:invarianza-autovalori}
...
\end{lemma}
\begin{proof}
	Sia $\beta\in V\setminus W$ e $g$ il polinomio $T$-conducente per $\beta\in W$: ciò implica che $g(T)\beta\in W$. % Ma \beta appartiene o no a W?

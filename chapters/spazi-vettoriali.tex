...
\begin{itemize}
	...
	%\item Dato lo spazio delle matrici quadrate $\mat_{n,n}(\R)$, l'insieme delle matrici dette \emph{diagonali}, ossia con componenti nulle tranne che sulla diagonale,
	%	\begin{equation*}
	%		D=\left\{
	%		\begin{pmatrix}
	%			a_{11}	&0		&\cdots	&0\\
	...
	%			0		&0		&\cdots	&a_{nn}
	%		\end{pmatrix}
	%		\colon a_{ii}\in\R\right\},
	%	\end{equation*}
	%	è un sottospazio vettoriale di $\mat_{n,n}(\R)$. Analogamente anche l'insieme matrici triangolari, ossia con tutti i termini sotto (o sopra) della diagonale esclusa nulli, è un sottospazio vettoriale di $\mat_{n,n}(\R)$.
...
\begin{teorema}
	...
	\begin{equation*}
		V_k\cap\sum_{j\in I\setminus\{k\}}{V_j}=\{0_V\}.
	\end{equation*}
\end{teorema}
\begin{proof}
	Se $V_k\cap\sum_{j\in I\setminus\{k\}}{V_j}\neq\{O_V\}$ per qualche $k\in I$, allora esisterebbe un elemento $  v_k\in V_k$ non nullo che è combinazione lineare di elementi dei restanti sottospazi, cioè $  v_k=  x_{i_1}+\dots+  x_{i_m}$ con $\{i_1,\dots,i_r\}\subseteq I\setminus\{k\}$.
	...
	Viceversa se abbiamo che $V_k\cap\sum_{j\in I\setminus\{k\}}{V_j}=\{0_V\}$, allora significa che l'unico vettore $  v$ che appartiene allo spazio considerato è il vettore nullo.
	Poichè è comunque uno spazio vettoriale, possiamo scrivere $0_V$ come combinazione lineare di elementi degli altri sottospazi, quindi la famiglia di sottospazi è libera.  
\end{proof}
